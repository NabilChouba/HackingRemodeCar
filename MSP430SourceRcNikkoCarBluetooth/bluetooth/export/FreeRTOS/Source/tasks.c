...
extern void sdkPlatformInit(void);
                     ...
                     portTickType xTimeIncrement)
{
    ...
    vTaskSuspendAll();
    {
        ...
        if (xTickCount < *pxPreviousWakeTime) {
             ...
             * either the wake time has overflowed, and/or the tick time is less
             * * than the wake time. */
            if ((xTimeToWake < *pxPreviousWakeTime)
                || (xTimeToWake > xTickCount)) {
                xShouldDelay = pdTRUE;
...
signed portBASE_TYPE xTaskRemoveFromEventList(const xList * const pxEventList)
{
     ...
     * modified instead. This means we can always expect exclusive access to the
     * * event list here. */
